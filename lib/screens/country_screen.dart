...
class _CountryScreenState extends State<CountryScreen> {
  ...
  void initState() {
    super.initState();
    _description = widget.country.name;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            Text(widget.country.id == 0 ? 'New country' : widget.country.name),
      ),
      body: Stack(
        children: [
          Column(
  ...
  void _save() {
    ...
    widget.country.id == 0 ? _addRecord() : _saveRecord();
  }
  ...
  bool _validateFields() {
    ...
    if (_description.isEmpty) {
      ...
      _descriptionError = 'You must enter a country name.';
    } else {
  ...
  _addRecord() async {
    ...
    Response response =
        await ApiHelper.post('/api/Countries/', request, widget.token.token);
  ...
  _saveRecord() async {
    ...
    Response response = await ApiHelper.put('/api/Countries/',
        widget.country.id.toString(), request, widget.token.token);
  ...
  void _deleteRecord() async {
    ...
    Response response = await ApiHelper.delete(
        '/api/Countries/', widget.country.id.toString(), widget.token.token);
