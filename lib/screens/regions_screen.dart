...
import 'package:madeupu_app/screens/region_screen.dart';
...
class _RegionsScreenState extends State<RegionsScreen> {
  ...
  Widget _getContent() {
    return _regions.isEmpty ? _noContent() : _getListView();
  }
  ...
  void _filter() {
    ...
    List<Region> filteredList = [];
    for (var region in _regions) {
      if (region.name.toLowerCase().contains(_search.toLowerCase())) {
        filteredList.add(region);
      }
    }

    setState(() {
      _regions = filteredList;
      _isFiltered = true;
    });
  ...
  void _goEdit(Region region) async {
    String? result = await Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => RegionScreen(
