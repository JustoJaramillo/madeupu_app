...
class ApiHelper {
  ...
  static Future<Response> getProjectById(Token token, String id) async {
    if (!_validToken(token)) {
      return Response(
          isSuccess: false,
          message:
    ...
    var url = Uri.parse('${Constants.apiUrl}/api/projects/$id');
    var response = await http.get(
      url,
      headers: {
        'content-type': 'application/json',
    ...
    var decodedJson = jsonDecode(body);
    return Response(isSuccess: true, result: Project.fromJson(decodedJson));
  }
}
